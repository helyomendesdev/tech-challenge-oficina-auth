...
- ZIP sem `tests/`, `.git/`, `.env`, caches, `.venv`, `dist/`, `build/`, arquivos `.pem`, `.key`, `.pyc` ou `.pyo`; os diretórios `.dist-info` permanecem, porque `scramp` lê os próprios metadados no import.
